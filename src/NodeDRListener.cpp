...
namespace NodeOpenDDS {
  ...
  Local<Object> copyToV8(const DDS::Time_t& src)
  {
    ...
    Nan::Set(stru, Nan::New<String>("nanosec").ToLocalChecked(), Nan::New(src.nanosec));
    return stru;
  }
...
#undef INT
    Nan::Set(stru, Nan::New<String>("valid_data").ToLocalChecked(), Nan::New(src.valid_data));
    return stru;
  }

  NodeDRListener::NodeDRListener(DDS::DomainParticipant* dp, const Local<Function>& callback)
      : dp_(dp), callback_(callback), vd_(0), async_uv_(this), unsubscribing_(false),
        unsubscribed_(false), receiving_samples_(false)
  {
  ...
  void NodeDRListener::async() // called from libuv event loop
  {
    ...
    try {
      ...
      dri->take(*this, DDS::NOT_READ_SAMPLE_STATE, DDS::ANY_VIEW_STATE, DDS::ANY_INSTANCE_STATE);
    } catch (...) {
    }
    lock.lock();
  ...
  void NodeDRListener::set_javascript_datareader(const Local<v8::Object>& js_dr)
  {
    ...
    OpenDDS::DCPS::DataReaderImpl* dri =
        dynamic_cast<OpenDDS::DCPS::DataReaderImpl*>(static_cast<DDS::DataReader*>(dr_obj));
    if (dri) {
  ...
  void NodeDRListener::push_back(const DDS::SampleInfo& src, const void* sample)
  {
    ...
    Local<Value> argv[] = {Nan::New(js_dr_), copyToV8(src),
                           vd_ && src.valid_data ? nvw_.get_result().As<Value>()
  ...
  void NodeDRListener::unsubscribe_now_i(std::unique_lock<std::mutex>& lock)
  {
    ...
    const DDS::ContentFilteredTopic_var cft = DDS::ContentFilteredTopic::_narrow(td);
    if (cft) {
