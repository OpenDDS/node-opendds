...
namespace Nan {
  namespace imp {
    ...
    template <> struct ToFactory<v8::BigInt> : ToFactoryBase<v8::BigInt>
    {
      static inline return_t convert(v8::Local<v8::Value> val)
      {
        ...
        return scope.Escape(
            val->ToBigInt(isolate->GetCurrentContext()).FromMaybe(v8::Local<v8::BigInt>()));
      }
    };
...
namespace NodeOpenDDS {
  ...
  NodeValueReader::NodeValueReader(v8::Local<v8::Object> obj) : current_index_(0), use_name_(false)
  {
  ...
  bool NodeValueReader::begin_struct_member(OpenDDS::XTypes::MemberId& member_id,
                                            const OpenDDS::DCPS::MemberHelper& helper)
  {
  ...
  bool NodeValueReader::member_has_value()
  {
    Nan::MaybeLocal<v8::Value> mlvai = current_property_name_.IsEmpty()
                                           ? Nan::Get(current_object_, current_index_)
                                           : Nan::Get(current_object_, current_property_name_);
    if (!mlvai.IsEmpty()) {
  ...
  bool NodeValueReader::begin_map(OpenDDS::XTypes::TypeKind /*key_kind*/,
                                  OpenDDS::XTypes::TypeKind /*value_kind*/)
  {
    return false;
  }
  ...
  {
    Nan::MaybeLocal<v8::Value> mlvai = use_name_ ? Nan::Get(current_object_, current_property_name_)
                                                 : Nan::Get(current_object_, current_index_);
    if (!mlvai.IsEmpty()) {
#ifdef HAS_BIGINT
  ...
  bool NodeValueReader::read_long_enum(ACE_CDR::Long& value,
                                       const OpenDDS::DCPS::EnumHelper& helper)
  {
    Nan::MaybeLocal<v8::Value> mlvai = use_name_ ? Nan::Get(current_object_, current_property_name_)
                                                 : Nan::Get(current_object_, current_index_);
    if (!mlvai.IsEmpty()) {
  ...
  bool NodeValueReader::read_bitmask(ACE_CDR::ULongLong& value,
                                     const OpenDDS::DCPS::BitmaskHelper& helper)
  {
    Nan::MaybeLocal<v8::Value> mlvai = use_name_ ? Nan::Get(current_object_, current_property_name_)
                                                 : Nan::Get(current_object_, current_index_);
