...
namespace NodeOpenDDS {
  ...
  template <typename T, typename U> void safe_assign(T& out, const U& in)
  {
    out = static_cast<T>(in);
  }

  template <typename U> void safe_assign(ACE_CDR::LongDouble& out, const U& in)
  {
  ...
  public:
    ...
    bool end_struct();
    bool begin_struct_member(OpenDDS::XTypes::MemberId& member_id,
  ...
  private:
    ...
    template <typename V, typename T> bool primitive_helper(T& value, ValueChecker checker)
    {
    ...
    bool primitive_helper(T& value, ValueChecker checker,
                          U (*str_conv)(const char*, char**, int base))
    {
      Nan::MaybeLocal<v8::Value> mlvai = current_property_name_.IsEmpty()
                                             ? Nan::Get(current_object_, current_index_)
                                             : Nan::Get(current_object_, current_property_name_);
      if (!mlvai.IsEmpty()) {
        ...
        if (lvai->IsString()) {
          ...
          safe_assign(
              value,
              (*str_conv)(*str, 0, std::string(*str).find("0x") != std::string::npos ? 16 : 10));
          return true;
        }
      }
      return false;
